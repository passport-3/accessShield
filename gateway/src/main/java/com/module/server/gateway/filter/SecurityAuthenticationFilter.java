...
class SecurityAuthenticationFilter implements AuthenticationWebFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
        return Mono.just(exchange)
                .filterWhen(this::checkIpWhitelist)
                .flatMap(this::extractAndValidateToken)
                .flatMap(auth -> {
                    exchange.getAttributes().put(SecurityContext.class.getName(),
                            SecurityContextImpl.builder().authentication(auth).build());
    ...
    private Mono<Authentication> extractAndValidateToken(ServerWebExchange exchange) {
        ...
        return getTokenFromRedis(token)
                ...
                .flatMap(validToken -> validateToken(validToken))
