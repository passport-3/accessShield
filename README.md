...
1. Gateway에 의존성 추가
    ```groovy
    dependencies {
        implementation project(':filter-module')
    }
    ```

2. Gateway 모듈에서 필터 사용
    ```java
    @Component
    public class AccessShield extends AbstractGatewayFilterFactory<AccessShield.Config> {
        private final MyCustomFilter myCustomFilter;

        public AccessShield(MyCustomFilter myCustomFilter) {
        ...
        public GatewayFilter apply(Config config) {
            return (exchange, chain) -> {
                return myCustomFilter.filter(exchange, chain);
            };
        }
    }
    ```
...
```mermaid
flowchart TD
    A[API 요청] --> B{IP White List 확인}
    B -->|허용되지 않은 IP| C[IP 에러 응답]
    B -->|허용된 IP| D{토큰 필요한 API인가?}
    
    D -->|No| E[API 처리 진행]
    D -->|Yes| F{Header에 토큰 존재?}
    
    F -->|No| G[토큰 없음 에러]
    F -->|Yes| H{Redis에 토큰 존재?}
    
    H -->|No| I[토큰 저장 없음 에러]
    H -->|Yes| J{Auth Server 토큰 검증}
    
    J -->|실패| K[토큰 검증 실패 에러]
    J -->|성공| L{사용자 권한 체크}
    
    J -->|토큰 만료| M{Redis에 Refresh Token 존재?}
    M -->|No| N[Refresh Token 없음 에러]
    M -->|Yes| O[Auth Server에 새 Access Token 요청]
    O --> P{재발급 성공?}
    P -->|Yes| L
    P -->|No| Q[토큰 재발급 실패 에러]
...
- **재사용 가능**
    - 모듈화된 필터를 통해 여러 프로젝트나 서비스에서 동일한 인증 및 보안 로직을 재사용할 수 있습니다.
...
- JWT
    - Access Token은 짧은 만료 시간을 가지며, 사용자 인증을 위해 자주 사용
    - Refresh Token은 상대적으로 긴 만료 시간을 가지며, Access Token이 만료되었을 때 새로운 Access Token을 발급받기 위해 사용
    - 이러한 방식은 보안성을 강화하고, 사용자가 지속적으로 인증된 상태를 유지하도록 함
    - JWT는 자체적으로 인증 정보를 포함하고 있어 추가적인 세션 저장소가 필요하지 않으며, 다양한 서비스에서 효율적인 인증을 가능하게 함

- Redis
    - AccessToken, RefreshToken 을 캐시하는 구조를 설계, Gateway 필터에서 Redis를 사용해 인증 토큰의 유효성을 확인하도록하여, Auth Server에 대한 요청을 최소화 함
    - TTL기능을 통해 스케줄링 구현 없이 토큰을 관리함
    - JWT 토큰의 인증 정보 캐싱을 통해 성능을 최적화하고, 서비스 간 빠른 데이터 공유가 가능하도록 함

- WebFlux
    - 필터 모듈에서 외부 서비스(Auth Server)에 대한 호출을 WebClient를 사용하여 수행하는데, 이는 WebFlux와 원활하게 통합되어 비동기적으로 요청을 보낼 수 있게 함. 이를 통해 인증서비스와의 통신을 효율적으로 처리
<br>
...
- 요청 header 중첩 문제
    ...
    - **해결 방안**:
        - 1차 → 필터 적용 경로를 조정하여 AuthFilter가 필요하지 않은 특정 서비스에 대해선 적용되지 않도록 설정했습니다.
        - 2차 → Spring Security Filter를 적용하여 특정 서비스에 대해선 적용되지 않도록 설정했습니다.
<br><br><br>
- User 서비스와 Auth 서비스 간의 DTO 구조 차이 문제
    ...
    - **원인** : User서비스에서 로그인시  `username`, `password`만 전달하도록 설정,  Auth서비스에서는 username,role,password를 필요로 하는 Dto를 받도록 설정 되어있었습니다.  따라서 
...
| --- | --- | --- | --- |
| 유희진 | 팀원 |  ▶ 멀티모듈 구성<br><br>▶ OAuth 2.0 기반 인증 시스템 구현<br>- 토큰 이중화(access/refresh)<br>- Auth server 분리<br><br>▶ 검증 기능 개발<br>- IP white list 확인<br>- 토큰 검증 | [https://github.com/heejin1023](https://github.com/heejin1023) | 
| 한수빈 | 팀원 |  ▶ 사용자 권한 기반 자원 접근 제한<br>- 각 API 경로에 대한 접근 권한을 미리 정의하고, Redis에 저장<br>- Gateway에서 Redis를 통해 역할 기반 권한을 처리<br><br>▶ 테스트를 위한 데모 사용자 서버 개발<br>- 회원가입, 로그인 구현<br>- 로그아웃 시 필요한 정보를 AUTH 서버에 전달 | [https://github.com/subinny2](https://github.com/subinny2) |
